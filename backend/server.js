...
// Session simulation (in-memory)
let session = null;
const ADMIN_USER = { username: 'admin', password: 'password' };

// Routes
app.get('/', (req, res) => res.send('Morgantown Boosters API is running'));
...
app.put('/events/:id', (req, res) => {
    const { title, description, date, location, goal } = req.body;
    db.run(
        `UPDATE events SET title=?, description=?, date=?, location=?, goal=? WHERE id=?`,
